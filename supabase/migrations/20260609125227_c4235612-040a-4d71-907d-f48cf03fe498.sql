CREATE OR REPLACE FUNCTION public.registrar_pagamento_cobranca(p_cobranca_id uuid, p_representante_id uuid, p_revendedora text, p_total_venda numeric, p_comissao_percentual numeric, p_comissao_valor numeric, p_valor_devido_empresa numeric, p_valor_pago_prestacao numeric, p_saldo_devedor numeric, p_forma_pagamento text, p_data_execucao date, p_codigo_nota_ref text, p_valor_nota numeric, p_forma_pagamento_1 text, p_valor_pagamento_1 numeric, p_forma_pagamento_2 text DEFAULT NULL::text, p_valor_pagamento_2 numeric DEFAULT NULL::numeric, p_devolveu_tudo boolean DEFAULT false, p_status_no_pagamento text DEFAULT NULL::text, p_novo_acumulado numeric DEFAULT NULL::numeric, p_novo_valor_previsto numeric DEFAULT NULL::numeric, p_novo_status text DEFAULT NULL::text, p_data_quitacao date DEFAULT NULL::date, p_data_agendada date DEFAULT NULL::date, p_valor_devolvido numeric DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO notas_promissorias (
    ...
    p_valor_nota, p_forma_pagamento_1::forma_pagamento, p_valor_pagamento_1,
