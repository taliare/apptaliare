import {
  Home, Users, Target, Upload, FileText, Calendar, CalendarCheck, Package,
  Factory, ShoppingBag, Scale, PackageCheck, UserPlus, Shield, BarChart3,
  TrendingUp, Receipt, FolderOpen, LineChart, ClipboardList, AlertTriangle,
  Wallet, BookOpen, ScanLine, ChevronDown,
} from "lucide-react";
...
export function AppSidebar() {
  ...
  const handleMouseEnter = () => { clearTimeout(hoverTimeout.current); setExpanded(true); };
  const handleMouseLeave = () => { hoverTimeout.current = setTimeout(() => setExpanded(false), 150); };
  ...
  useEffect(() => {
    ...
    setOpenMap((prev) => (prev[activeCategoryId] ? prev : { ...prev, [activeCategoryId]: true }));
  }, [activeCategoryId]);
  useEffect(() => {
  ...
  return (
    <aside
      ...
      className={cn(
        "fixed top-14 left-0 z-40 h-[calc(100vh-3.5rem)] flex flex-col",
        "bg-sidebar border-r border-sidebar-border transition-[width] duration-200 ease-out",
        expanded ? "w-56 shadow-xl" : "w-16",
      )}
    >
      <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar px-2 py-4">
        {!expanded ? (
            ...
            {categories.flatMap((c) => c.items).map((item) => {
              ...
              return (
                <NavLink
                  ...
                  to={item.route}
                  title={item.label}
                  className="flex items-center justify-center w-full px-2 py-2 rounded-lg text-sidebar-foreground/70 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground transition-all duration-200"
      ...
      <div className="flex-shrink-0 border-t border-sidebar-border/50 py-2">
        {expanded && (
          <div className="px-3 py-1 text-center">
            <p className="text-[10px] text-sidebar-foreground/30 uppercase tracking-widest">TALIARE</p>
    ...
    <div className="flex flex-col">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] uppercase tracking-widest font-medium">{category.label}</span>
        </div>
              ...
              return (
                <li key={item.key}>
                  ...
                  >
                    ...
                    {showBadge && (
                      <Badge variant="secondary" className="text-[10px] px-2 py-0 bg-primary/20 text-primary border-0">
                        {newLeadsCount}
                      </Badge>
                    )}
                  </NavLink>
